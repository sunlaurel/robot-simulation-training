...
class MultiLayer(nn.Module):
  def __init__(self, input_size, hidden_layer1, hidden_layer2, output_size):
    ...
    self.linear1 = nn.Linear(input_size, hidden_layer1)  # Linear layer: input_size -> hidden_layer
  ...
  def forward(self, x):              # Forward pass which defines how the layers relate the input x to the output out
    ...
    out = self.linear3(x)
    out = out.view(batch_size, 4, -1) # Reshape the output to (batch_size, 2, future_time_steps)
    return out
