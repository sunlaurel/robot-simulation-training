...
class MultiLayer(nn.Module):
  def __init__(self, input_size, hidden_layer1, hidden_layer2, output_size):
    ...
    self.linear1 = nn.Linear(input_size * 4, hidden_layer1)  # Linear layer: input_size -> hidden_layer
    self.linear2 = nn.Linear(hidden_layer1, hidden_layer2)  # Linear layer: hidden_layer -> num_classes
    self.linear3 = nn.Linear(hidden_layer2, output_size * 2)  # Linear layer: hidden_layer -> num_classes
...
class MultiLayer2(nn.Module):
  def __init__(self, input_size, hidden_layer1, hidden_layer2, hidden_layer3, hidden_layer4, output_size):
    super().__init__()                                 # Inherited from the parent class nn.Module
    self.linear1 = nn.Linear(input_size * 2, hidden_layer1)
    self.linear2 = nn.Linear(hidden_layer1, hidden_layer2)
    self.linear3 = nn.Linear(hidden_layer2, hidden_layer3)
    self.linear4 = nn.Linear(hidden_layer3, hidden_layer4)
    self.linear5 = nn.Linear(hidden_layer4, output_size * 2)
