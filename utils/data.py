...
def ConvertAbsoluteToRobotFrame(X_past, R_past, heading, future_pos):
    ...
    # breakpoint()
    relative_vectors_w = X_past - R_past[:, None]
    ...
    for i in range(0, X_past.shape[1]):
        ...
        # X_past[:, i] = rotation_matrix @ X_past[:, i]
        # R_past[:, i] = rotation_matrix @ R_past[:, i]

        # if i == len(R_tangent[0]) - 1:
        #     future_pos = rotation_matrix @ future_pos
        #     ax[1].scatter(future_pos[0], future_pos[1], label="Future in robot")
...
def ConvertRobotFrameToAbsolute(heading, future_pos_r):
    R_tangent = heading
    # if np.linalg.norm(R_tangent) > 1e-04:
    ...
    # breakpoint()
    future_pose_w = rotation_matrix_r_w @ future_pos_r.float()

    return future_pose_w
